...
class GrabCAD(Vendor):
    ...
    # TODO: use kwargs for anything after query 
    def search(self, query: str, sort: str = "most_downloaded", allowed_types: list = ["step-slash-iges"]) -> list[dict] | int:
        url = f"https://grabcad.com/community/api/v1/models?query={query}&sort={sort}&softwares={','.join(allowed_types)}"
        headers = {
    ...
    def __reformat__(self, item: dict, allowed_types: list) -> dict:
        ...
        files_url = url = f"https://grabcad.com/community/api/v1/models/{item['cached_slug']}/files"
        headers = {
            "User-Agent": 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:154.0) Gecko/20100101 Firefox/154.0'
        }
